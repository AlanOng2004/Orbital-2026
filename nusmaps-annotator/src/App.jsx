...
// TODO:
// 1. Add floor colours for nodes and edges

// Math helper for pgRouting "cost"
const calculateDistance = (x1, y1, x2, y2) => {
  return (
    Math.round(Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2)) * 100) /
    100
  );
};
...
export default function App() {
  ...
  const currentNode = useRef(0);
  const currentFloorplan = useRef(0);

  const handleImageUpload = (e) => {
    if (e.target.files[0]) {
      setImageUrl(URL.createObjectURL(e.target.files[0]));
      setZoom(1);
      currentFloorplan.current += 1;
    }
  };
  ...
  const handleSvgClick = (e) => {
    ...
    const newNode = {
      id: currentNode.current,
      floor: currentFloorplan.current,
  ...
  const finishEdge = (e, targetNode) => {
    ...
    setEdges([
      ...edges,
      { source: draftEdge.sourceId, target: targetNode.id, cost },
    ]);
    setDraftEdge(null);
  };
  ...
  const generateSQL = () => {
    ...
    if (edges.length > 0) {
      ...
      const edgeValues = edges
        .map((e) => `(${e.source}, ${e.target}, ${e.cost})`)
        .join(",\n");
      sql += edgeValues + `;\n`;
    }
    setGeneratedSQL(sql);
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "5px", marginLeft: "0px" }}>
            <input
              type="number"
              placeholder="Floor ID"
              onChange={(e) => {
                currentFloorplan.current = parseInt(e.target.value) || 0;
              }}
              style={{
                width: "70px",
                padding: "5px",
            ...
            <input
              ...
              placeholder="Node ID"
              onChange={(e) => {
                currentNode.current = parseInt(e.target.value) || 0;
              }}
              style={{
                width: "70px",
                padding: "5px",
          ...
          {imageUrl && (
            ...
            >
              ...
              <svg
                ...
                onMouseMove={updateEdge}
                onMouseUp={() => setDraftEdge(null)}
                onMouseLeave={() => setDraftEdge(null)}
              >
                {edges.map((e, i) => {
                ...
                {nodes.map((n) => (
                  <circle
                    ...
                    cy={n.y}
                    r={6 / zoom}
                    fill={mode === "ADD_NODE" ? "green" : "orange"}
                    onMouseDown={(e) => startEdge(e, n)}
                    onMouseUp={(e) => finishEdge(e, n)}
                    style={{ cursor: "pointer" }}
                  >
